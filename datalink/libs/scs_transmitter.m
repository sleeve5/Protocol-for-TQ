...
function encoded_symbol_stream = scs_transmitter(frame_queue, sim_params)
    % 1. 初始化与参数校验
    % 如果未提供参数结构体，初始化为空结构体
    if nargin < 2
        sim_params = struct();
    ...
    if sim_params.AcqSeqLen > 0
        ...
        stream_buffer{buffer_idx} = generate_idle_sequence(sim_params.AcqSeqLen);
    ...
    if num_frames > 0
        ...
        for i = 1:num_frames
            ...
            if (i < num_frames) && (sim_params.InterFrameGap > 0)
                stream_buffer{buffer_idx} = generate_idle_sequence(sim_params.InterFrameGap);
                buffer_idx = buffer_idx + 1;
            end
        end
    else
